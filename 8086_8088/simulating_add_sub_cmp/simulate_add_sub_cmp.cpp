...
// [1] -> ZF
char flags[2] = {0, 0};
char *flagName[2] = {"SF", "ZF"};

int16_t word_table[8] = {0, 0, 0, 0, 0, 0, 0, 0};
int8_t byte_table[8] = {0, 0, 0, 0, 0, 0, 0, 0};
...
void print_final_registers()
{
    ...
    for (int i = 0; i < 8; ++i)
    {
        if (word_table[i] != 0)
        {
            printf("\t%s: 0x%.4x (%d)\n", namesWord[i], word_table[i], word_table[i]);
        }
    }
...
void displayValueOfRegister(char des, int16_t source, char instr_type)
{
    int16_t beforeAssign = word_table[des];
    int16_t value;
    if (instr_type == SUB)
    {
        value = word_table[des] - source;
        word_table[des] = value;
        printf(" ; %s:0x%x->0x%x", namesWord[des], beforeAssign, word_table[des]);
    }
    else if (instr_type == ADD)
    {
        value = word_table[des] + source;
        word_table[des] = value;
        printf(" ; %s:0x%x->0x%x", namesWord[des], beforeAssign, word_table[des]);
...
// Only using for sub/add/cmp
void registerToRegister(uint8_t buffer, FILE *asmFile, char ins_type)
{
    char d = buffer & GET_D;
    char w = buffer & GET_W;
    uint8_t nextBuffer;
    ...
    if (mod == 0b11)
    {
        if (d)
        {
            ...
            displayValueOfRegister(reg, word_table[rm], ins_type);
        }
        else
        {
            printf("%s, %s", rmValue, regValue);
            displayValueOfRegister(rm, word_table[reg], ins_type);
        }
    }
    else
    {
...
void immediateToRegisterMem(uint8_t buffer, FILE *asmFile)
{
    ...
    if (s == 0 && w == 1)
    {
        ...
        printf(", %d", data_16);
        displayValueOfRegister(rm, data_16, inst_type);
    }
    else
    {
        fread(&data_8, sizeof(int8_t), 1, asmFile);
        printf(", %d", data_8);
        displayValueOfRegister(rm, data_8, inst_type);
    }
}
        ...
        while (!feof(asmFile))
        {
            ...
            if (buffer >> 2 == REGISTER_TO_REGISTER)
            {
                ...
                if (mod == 0b11)
                {
                    if (movd)
                    {
                        int16_t beforeAssign = word_table[reg];
                        word_table[reg] = word_table[rm];
                        printf("%s, %s", regValue, rmValue);
                        printf(" ; %s:0x%x->0x%x", regValue, beforeAssign, word_table[reg]);
                    }
                    else
                    {
                        int16_t beforeAssign = word_table[rm];
                        word_table[rm] = word_table[reg];
                        printf("%s, %s", rmValue, regValue);
                        printf(" ; %s:0x%x->0x%x", rmValue, beforeAssign, word_table[rm]);
                    }
                }
                else
                {
            ...
            else if (buffer >> 4 == IMMEDIATE_TO_REGISTER)
            {
                ...
                if (movw)
                {
                    ...
                    word_table[reg] = nextBuffer;
                    printf("%s, %d", namesWord[reg], nextBuffer);
                    printf(" ; %s:0x%.4x->0x%.4x", namesWord[reg], beforeAssign, word_table[reg]);
                }
                else
                {
                    int8_t beforeAssign = byte_table[reg];
                    int8_t nextBuffer;
                    fread(&nextBuffer, sizeof(int8_t), 1, asmFile);
                    byte_table[reg] = nextBuffer;
                    printf("%s, %d", namesByte[reg], nextBuffer);
                    printf(" ; %s:0x%x->0x%x", namesByte[reg], beforeAssign, byte_table[reg]);
                }
            }
            ...
            {
                printf("add ");
                registerToRegister(buffer, asmFile, ADD);
            }
            ...
            {
                printf("sub ");
                registerToRegister(buffer, asmFile, SUB);
            }
            ...
            {
                printf("cmp ");
                registerToRegister(buffer, asmFile, CMP);
            }
